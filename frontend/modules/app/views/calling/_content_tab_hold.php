...
<div class="panel-body" style="border-top: 1px solid #e4e5e7;">
    <?php
    echo Table::widget([
        'tableOptions' => ['class' => 'table table-hover', 'id' => 'tb-hold', 'cellpadding' => 1, 'cellspacing' => 1],
        'beforeHeader' => [
            [
                'columns' => [
        ...
        'datatableOptions' => [
            "clientOptions" => [
                "dom" => "<'row'<'col-sm-5'f><'col-sm-2 toolbar-hold'><'col-sm-5'l B>><'row'<'col-xs-12 col-sm-12 col-md-12'tr>><'row'<'col-sm-6'i><'col-sm-6'p>>",
                "ajax" => [
                    "url" => Url::base(true) . "/app/calling/data-que-hold",
                    "type" => "POST",
                    "data" => ["data" => $modelProfile,'formData' => $formData],
                    "complete" => new JsExpression('function(qXHR, textStatus ){
                        var api = $(\'#tb-hold\').DataTable();
                        setTimeout(function(){
                ...
                "initComplete" => new JsExpression('function ( settings ) {
                    var api = this.api();
                    $("div.toolbar-hold").html(\'<span class="badge badge-warning text-center"><i class="pe-7s-ticket"></i> '.Yii::t('frontend', 'Queue Hold').'</span>\');
                }'),
                "columnDefs" => [
